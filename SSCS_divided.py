...
#
# Outputs:
# Return files for specific REGION of bam -> subsequently merge bams
# Note: stats written to qsub output and plot cannot be generated for each separate bam 
...
#
# python3 SSCS_divided.py --Ncutoff 0.3 --cutoff 0.7 --infile /Users/nina/Desktop/Ninaa/PughLab/Molecular_barcoding/code/PL_consensus/test/MEM-001_KRAS.bam --SSCS_outfile MEM-001_KRAS.sscs.bam --region chr12_0_3300000
#
###############################################################
...
def consensus_maker(readList, readLength, cutoff):
    ...
    for i in range(readLength):
        ...
        try:
            ...
            max_qual = max(quality_score[max_nuc])
...
def main():
    ...
    parser.add_argument("--region", action = "store", dest="region", help="region of bam to fetch with pysam", required = True)
    ...
    singleton_bam = pysam.AlignmentFile('{}_singleton.bam'.format(args.SSCS_outfile.split('.')[0]), "wb", template = bamfile)
    ...
    pos = args.region.split('_')
    bamLines = bamfile.fetch(reference = pos[0], start = int(pos[1]), end = int(pos[2]))
...
Singletons: {} \n
Rescued reads: {} \n
'''.format(counter, bad_reads, SSCS_reads, singletons, 0)
    ...
    ## CAN'T create plots as tag family is only for each arm
